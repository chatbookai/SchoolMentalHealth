...
//Except CSRS
global $ExceptCsrf;
$ExceptCsrf[] = "/api/apps/apps_9.php";
$ExceptCsrf[] = "/api/apps/apps_18.php";
$ExceptCsrf[] = "/api/apps/apps_378.php"; //老师模拟心理测评
$ExceptCsrf[] = "/api/apps/apps_380.php"; //心理测评结果
$ExceptCsrf[] = "/api/apps/apps_382.php";
$ExceptCsrf[] = "/api/apps/apps_384.php"; //学生心理测评记录
...
function CheckAuthUserRoleHaveMenu($FlowId, $MenuPath='')  {
	...
	global $db;
	$HavePermisstion = 0;
	if($GLOBAL_USER->USER_ID!="" && $GLOBAL_USER->type=="User")   {
	...
	global $SystemMark;
	if($HavePermisstion==0 && $SystemMark=="Individual" && $GLOBAL_USER->type=="User")    {
		$RS['status'] 		= "ERROR";
        $RS['msg'] 			= "Not Have Permisstion";
...
function InsertOrUpdateTableByArray($Tablename, $Element, $primarykey="username,department", $Debug=0, $InsertMode='InsertOrUpdate')			{
	...
	if($InsertMode=='Insert')
	{
		...
		if($NUM==0)		{
			...
			if($Debug==0)		{
				$rs = $db->Execute($sql);
				$Insert_ID = $db->Insert_ID($Tablename);
                return [$rs, $sql, $Insert_ID];
			}
			else	{
	...
	{
		$WHERESQL_TEXT = join(' and ',$WHERESQL);
		$sql	= "select id from $Tablename where $WHERESQL_TEXT";
		$rs		= $db->Execute($sql);
		$NUMId	= (int)$rs->fields['id'];
		if($NUMId==0)		{
			$sql	= "insert into $Tablename (`".join('`,`',$KEYS)."`) values('".join("','",$VALUES)."')";
			if($Debug==0)		{
			...
			$sql	= "update $Tablename set ".join(',',$UPDATESQL)." where id='$NUMId'";
			if($Debug==0)		{
