...
if( ($_GET['action']=="add_default_data") && $_POST['DictMark']!="")  {
    ...
    for($i=0;$i<sizeof($EnglishNameArray);$i++)    {
        ...
        if(1)   {
            [$rs,$sql, $InsertRecordId] = InsertOrUpdateTableByArray("form_formdict",$FieldsArray,"DictMark,EnglishName",0,"Insert");
        }
    }
    if($Exec_Total) {        
        $RS['status'] = "OK";
...
if( ($_GET['action']=="edit_default_data") && $_GET['id']!="")  {
    ...
    $FieldsArray['ExtraControl']    = $_POST['ExtraControl'];
    if(1)   {
        [$rs,$sql, $InsertRecordId] = InsertOrUpdateTableByArray("form_formdict",$FieldsArray,"id",0,"Update");
        if($rs->EOF) {
            $RS['status'] = "OK";
            $RS['msg'] = __("Submit Success");
            print json_encode($RS);
