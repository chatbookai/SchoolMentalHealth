...
if( ($_GET['action']=="add_default_data") && $_POST['FieldName']!="" && $externalId!="")  {
    ...
    for($i=0;$i<sizeof($FieldNameArray);$i++)    {
        ...
        if(1)   {
            [$rs,$sql, $InsertRecordId] = InsertOrUpdateTableByArray("form_configsetting",$FieldsArray,"FlowId,FieldName",0,"Insert");
            if(!$rs->EOF) {
                $Exec_Total   = 0;
                $RS = [];
                $RS['status'] = "ERROR";
...
if( ($_GET['action']=="edit_default_data") && $_GET['id']!="" && $externalId!="" && $_POST['FieldName']!="")  {
    ...
    if(1)   {
        [$rs,$sql, $InsertRecordId] = InsertOrUpdateTableByArray("form_configsetting",$FieldsArray,"id",0,"Update");
        if($rs->EOF) {
            $RS['status'] = "OK";
            $RS['msg'] = __("Submit Success");
            print json_encode($RS);
