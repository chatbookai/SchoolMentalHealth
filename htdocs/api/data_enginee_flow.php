        ...
        if($IsExecutionSQL)    {
            ...
            switch($Import_Rule_Method) {
                case 'BothInsertAndUpdate':
                    [$rs,$sql, $InsertRecordId] = InsertOrUpdateTableByArray($TableName,$Element,join(',',$ImportUniqueFields),0,'InsertOrUpdate');
                    $sqlList[] = $sql;
                    break;
                case 'OnlyUpdate':
                    [$rs,$sql, $InsertRecordId] = InsertOrUpdateTableByArray($TableName,$Element,join(',',$ImportUniqueFields),0,'Update');
                    $sqlList[] = $sql;
                    break;
                case 'OnlyInsert':
                    [$rs,$sql, $InsertRecordId] = InsertOrUpdateTableByArray($TableName,$Element,join(',',$ImportUniqueFields),0,'Insert');
                    $sqlList[] = $sql;
                    break;
            }
            if($rs->EOF) {
...
if( $_GET['action']=="edit_default_data" && in_array('Edit',$Actions_In_List_Row_Array) && $_GET['id']!="" && $TableName!="")  {
    ...
    if($IsExecutionSQL || $IsExecutionSQLChildTable)   {
        [$Record, $sql, $InsertRecordId]  = InsertOrUpdateTableByArray($TableName, $FieldsArray, 'id', 0, "Update");
        if($Record->EOF) {
...
if( ( ($_GET['action']=="edit_default"&&in_array('Edit',$Actions_In_List_Row_Array))  ) && $_GET['id']!="")  {
    ...
    if($_GET['IsGetStructureFromEditDefault']==1)  {
        ...
        $edit_default['model']          = $SettingMap['EditModel'];
    }
    //Relative Child Table Support
    $Relative_Child_Table                   = $SettingMap['Relative_Child_Table'];
    $Relative_Child_Table_Field_Name        = $SettingMap['Relative_Child_Table_Field_Name'];
...
$RS['edit_default']['model']            = $SettingMap['EditModel'];
