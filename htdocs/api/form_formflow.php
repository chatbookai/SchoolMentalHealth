...
if( ($_GET['action']=="add_default_data") && $_POST['FlowName']!="" && $externalId!="")  {
    ...
    if(1)   {
        [$rs,$sql, $InsertRecordId] = InsertOrUpdateTableByArray($TableName,$FieldsArray,"FormId,Step",0,"Insert");
        if($rs->EOF) {
            $RS['status'] = "OK";
...
$EditModel[] = ['value'=>'Form', 'label'=>'Form'];
$EditModel[] = ['value'=>'Loop', 'label'=>'Loop'];
$edit_default_2['Tip_In_Interface'][] = ['name' => "EditModel", 'show'=>true, 'type'=>'select', 'options'=>$EditModel, 'label' => __("EditModel"), 'value' => $EditModel[0]['value'], 'placeholder' => "", 'helptext' => "", 'rules' => ['required' => true, 'disabled' => false, 'xs'=>12, 'sm'=>4]];
...
if(($_GET['action']=="edit_default_6_data") && $id!="")     {
    ...
    [$rs,$sql, $InsertRecordId] = InsertOrUpdateTableByArray("form_formflow",$FieldsArray,'FormId,Step',0);
    if($rs->EOF) {
        $RS['status'] = "OK";
        $RS['_POST'] = $_POST;
        $RS['msg'] = __("Submit Success");
...
if(($_GET['action']=="edit_default_1_data" || $_GET['action']=="edit_default_2_data" || $_GET['action']=="edit_default_3_data" || $_GET['action']=="edit_default_4_data" || $_GET['action']=="edit_default_5_data") && $id!="")     {
    ...
    if($_POST['Menu_One']!=""&&$_POST['Menu_Two']!=""&&$id!="")   {
        ...
        $FieldsArray['Creator']        = "admin";
        $FieldsArray['CreateTime']     = date("Y-m-d H:i:s");
        [$rs,$sql, $InsertRecordId] = InsertOrUpdateTableByArray("data_menutwo",$FieldsArray,'FlowId',0);
