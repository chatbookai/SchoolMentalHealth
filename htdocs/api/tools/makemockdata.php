...
function 生成学生积分测试数据() {
    ...
    for($i=0;$i<90;$i++)  {
        ...
        if($Element['学号']!=""&&$Element['一级指标']!="")  {
            [$Record, $sql, $InsertRecordId] = InsertOrUpdateTableByArray("data_deyu_geren_record", $Element, '积分编码,积分时间,学号', 0);
            print $sql."<BR>";
        }
    }
...
function 生成班级评价测试数据() {
    ...
    for($i=0;$i<60;$i++)  {
        ...
        if($Element['班级']!=""&&$Element['一级指标']!="")  {
            [$Record, $sql, $InsertRecordId] = InsertOrUpdateTableByArray("data_deyu_banji_record", $Element, '积分项目,积分编码,积分时间,班级', 0);
            print $sql."<BR>";
        }
    }
