...
foreach($rs_a as $Line) {
    ...
    //$db->Execute($sql);
    //[$rs,$sql, $InsertRecordId] = InsertOrUpdateTableByArray("form_formfield_logictype",$FieldsArray,"FieldType",0,"Insert");
    print $sql."<BR>";
}
exit;
*/
...
for($i=1;$i<sizeof($data);$i++)                 {
    ...
    if($data[$i][0]!="" && 0)    {
        [$rs,$sql, $InsertRecordId] = InsertOrUpdateTableByArray("form_formdict",$FieldsArray,"DictMark,ChineseName",0);
        if(!$rs->EOF&&!$rs) {
...
foreach($字典标识 as $Label=>$Value) {
    ...
    if(0)    {
        [$rs,$sql, $InsertRecordId] = InsertOrUpdateTableByArray("form_formfield_showtype",$FieldsArray,"Name",0,"Insert");
        if(!$rs->EOF&&!$rs) {
...
for($i=1;$i<sizeof($data);$i++)   {
    ...
    if(1)   {
        //print_R($FieldsArray);
        [$rs,$sql, $InsertRecordId] = InsertOrUpdateTableByArray("form_formfield",$FieldsArray,"FormId,FieldName",0,"Insert");
        if(!$rs->EOF&&!$rs) {
